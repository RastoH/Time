...
typedef void (*setExternalTime)();     // not used in this version
...
int     year(time_t t);    // the year for the given time
  ...
  /** get the seconds since Jan 01, 1970, 00:00 (GMT + 8) timeZone */
time_t now();              // return the current time as seconds since Jan 1 1970 
  /** get the seconds since Jan 01, 1970, 00:00 (GMT + 8) - timeZone */
time_t Now();              // fast, without sync from sync provider
...
void    setSyncReceiver(setExternalTime setTimeFunction); // identify the external time provider
